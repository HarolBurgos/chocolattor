"""
modelos.py  -  Clases de datos Chocolattor v5
Extraidas de AppChocolate_v5.py (version escritorio)
"""
import math
...
class InsumoMP:
    def __init__(self, nombre, cantidad, unidad, costo_unit):
        self.nombre = nombre; self.cantidad = cantidad
        self.unidad = unidad; self.costo_unit = costo_unit

    def costo_total(self): return self.cantidad * self.costo_unit
...
class MateriaPrima:
    ...
    def __init__(self, nombre, batch_kg=30.0, rendimiento=75.0):
        self.nombre = nombre; self.batch_kg = batch_kg
        self.rendimiento = rendimiento; self.insumos = []

    def costo_total_batch(self): return sum(i.costo_total() for i in self.insumos)

    def costo_por_kg(self):
        if self.batch_kg <= 0 or self.rendimiento <= 0: return 0.0
        return self.costo_total_batch() / self.batch_kg / (self.rendimiento / 100.0)

    def costo_por_gramo(self): return self.costo_por_kg() / 1000.0
...
class InsumoGeneral:
    ...
    def __init__(self, nombre, cantidad, unidad, costo_total):
        self.nombre = nombre; self.cantidad = cantidad
        self.unidad = unidad; self.costo_total = costo_total
...
class CostoFijo:
    ...
    def __init__(self, nombre, monto, categoria="General"):
        self.nombre = nombre; self.monto = monto; self.categoria = categoria
...
class OtroGasto:
    def __init__(self, nombre, costo_unit):
        self.nombre = nombre; self.costo_unit = costo_unit
...
class ItemFormulacion:
    def __init__(self, nombre, proporcion_pct):
        self.nombre = nombre; self.proporcion_pct = proporcion_pct
...
class Presentacion:
    def __init__(self, nombre, peso_g):
        self.nombre = nombre; self.peso_g = peso_g
        self.formulacion = []; self.otros_gastos = []
        self.cantidad_mensual = 0.0; self.iva_pct = 0.0; self.ganancia_pct = 0.0
    ...
    def costo_variable_insumos(self, mps, igs):
        return sum((item.proporcion_pct / 100.0) * self.peso_g * self.cpg(item.nombre, mps, igs)
                   for item in self.formulacion)

    def costo_otros(self): return sum(g.costo_unit for g in self.otros_gastos)
    ...
    def margen_contrib(self, mps, igs, total_cf):
        return self.precio_venta(mps, igs, total_cf) - self.costo_variable_unit(mps, igs)

    def to_dict(self):
        return {"nombre": self.nombre, "peso_g": self.peso_g,
                ...
                "iva_pct": self.iva_pct, "ganancia_pct": self.ganancia_pct}
